...
for t in range(0, step, dt):
    ...
    Ra_cfrp = g * beta / nu / alpha * (T_cfrp - T_bottom) * L_charac ** 3
    Nu_cfrp_free = 0.15 * Ra_cfrp ** (1 / 3)
    h_cfrp_free = Nu_cfrp_free * k / L_charac
    q_cfrp_free_conv = h_cfrp_free * A * (T_cfrp - T_bottom)
    ...
    q_cfrp_forced_conv = h_cfrp_forced * A * (T_cfrp - T_bottom)
...
print(round(q[1][max_index]), "Watts released through top radiation", round(q[1][max_index]/q[0][max_index],2), "%")
print(round(q[2][max_index]), "Watts released trough top free conv", round(q[2][max_index]/q[0][max_index],2), "%")
print(round(q[3][max_index]), "Watts released trough top forced conv", round(q[3][max_index]/q[0][max_index],2), "%")
print(round(q[4][max_index]), "Watts released through bottom radiation", round(q[4][max_index]/q[0][max_index],2), "%")
print(round(q[5][max_index]), "Watts released trough bottom free conv", round(q[5][max_index]/q[0][max_index],2), "%")
print(round(q[6][max_index]), "Watts released trough bottom forced conv", round(q[6][max_index]/q[0][max_index],2), "%")
...
plt.show()
"""
maxpower_index = np.argmax(Power_list)
print(Power_list[maxpower_index])
print(I_new[maxpower_index])